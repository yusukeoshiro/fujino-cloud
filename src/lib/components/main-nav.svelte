<script lang="ts">
	...
	$effect(() => {
		...
		const found = menus.find((m) =>
			page.params.oid && currentPath.startsWith(m.path(page.params.oid))
		);
		if (found) {
		...
		>
			<Tabs.List class="bg-transparent p-0">
				{#each menus as menu (menu.value)}
					<Tabs.Trigger
						value={menu.value}
						class="relative rounded-full px-4 py-2 text-sm font-medium text-muted-foreground transition-all data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-sm hover:text-foreground data-[state=active]:hover:text-white"
					>
						{$t(menu.labelKey)}
					</Tabs.Trigger>
				{/each}
